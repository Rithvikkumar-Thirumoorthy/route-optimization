#!/usr/bin/env python3
"""
Run pipeline for specific agents and days only
Agents: 914 (2025-09-23), 10551 (2025-09-02), SK-PMS (2025-09-27)
"""

from scalable_route_optimizer import ScalableRouteOptimizer
import sys

def run_specific_agents():
    """Run pipeline for specific agents and days only"""
    print("RUNNING PIPELINE FOR SPECIFIC AGENTS")
    print("=" * 45)
        ...
        for agent_id, route_date in specific_agents:
            ...
            try:
                ...
                if result and result[0][0] > 0:
                    ...
                    # Process all agents, regardless of customer count
                    if customer_count >= 60:
                        print(f"  Agent has {customer_count} customers (>=60) - processing without adding prospects")

                    # Get customers for this agent-date (modified for Code column)
                    customers_query = f"""
                    ...
                    if existing_customers is not None and not existing_customers.empty:
                        ...
                        for _, customer in final_route.iterrows():
                            ...
                            route_data = {
                                'salesagent': str(agent_id),  # Use agent_id instead of sales_agent
