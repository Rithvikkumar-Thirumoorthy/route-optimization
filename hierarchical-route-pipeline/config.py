...
# ============================================================================
# ORS (OpenRouteService) API CONFIGURATION
# ============================================================================

ORS_CONFIG = {
    'enabled': os.getenv('ORS_ENABLED', 'True') == 'True',
    'matrix_endpoint': os.getenv('ORS_MATRIX_ENDPOINT', 'http://localhost:8080/ors/v2/matrix/driving-car'),
    'api_key': os.getenv('ORS_API_KEY', None),  # Optional API key for authenticated requests
    'timeout': int(os.getenv('ORS_TIMEOUT', '30')),  # Request timeout in seconds
    'use_cache': True,  # Cache distance matrix responses
    'fallback_to_haversine': True,  # Fallback to haversine if ORS fails
}
...
def validate_config():
    ...
    if ORS_CONFIG['enabled']:
        if not ORS_CONFIG['matrix_endpoint']:
            raise ValueError("ORS_MATRIX_ENDPOINT must be configured when ORS is enabled")
        if ORS_CONFIG['timeout'] <= 0:
            raise ValueError("ORS_TIMEOUT must be positive")
...
def print_config():
    ...
    print(f"ORS API: {'Enabled' if ORS_CONFIG['enabled'] else 'Disabled (using Haversine)'}")
    if ORS_CONFIG['enabled']:
        print(f"  Endpoint: {ORS_CONFIG['matrix_endpoint']}")
        print(f"  Timeout: {ORS_CONFIG['timeout']}s")
        print(f"  Caching: {'Enabled' if ORS_CONFIG['use_cache'] else 'Disabled'}")
