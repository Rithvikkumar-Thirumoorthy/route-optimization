...
class HierarchicalMonthlyRoutePipelineProcessor:
    def __init__(self, batch_size=50, max_workers=4, start_lat=None, start_lon=None, distributor_id=None):
        ...
        self._ors_matrix_cache = {}  # Cache ORS distance matrix responses
    ...
    def get_ors_distance_matrix(self, locations):
        ...
        Args:
            locations: List of [longitude, latitude] pairs (ORS uses lon,lat order!)
                      First location should be the starting point (distributor)
        ...
        try:
            ...
            # Check cache first (thread-safe)
            if config.ORS_CONFIG['use_cache']:
                with self._cache_lock:
                    if cache_key in self._ors_matrix_cache:
            ...
            # ORS expects locations in [longitude, latitude] format
            request_body = {
                "locations": locations,
                "metrics": ["distance"],
                "units": "km"
            ...
            if response.status_code == 200:
                ...
                # Extract distance matrix (ORS returns in meters, convert to km)
            ...
            self.logger.error(f"ORS API timeout after {config.ORS_CONFIG['timeout']}s")
            return None
        except requests.exceptions.ConnectionError:
            self.logger.error(f"ORS API connection error - is the service running at {config.ORS_CONFIG['matrix_endpoint']}?")
            return None
        except Exception as e:
    ...
    def calculate_distance(self, lat1, lon1, lat2, lon2):
        ...
        Uses ORS API if enabled, otherwise falls back to Haversine
        ...
        if config.ORS_CONFIG['enabled']:
            # Try ORS API first
            locations = [
                [lon1, lat1],  # ORS uses [lon, lat] order
                [lon2, lat2]
            ]

            distance_matrix = self.get_ors_distance_matrix(locations)
            ...
            if config.ORS_CONFIG['fallback_to_haversine']:
                self.logger.warning("ORS API failed, falling back to Haversine distance")
                return self.haversine_distance(lat1, lon1, lat2, lon2)
            else:
                raise Exception("ORS API failed and fallback to Haversine is disabled")
        else:
            # ORS disabled, use haversine
            return self.haversine_distance(lat1, lon1, lat2, lon2)
    ...
    def find_nearby_prospects_by_location(self, db, distributor_id, agent_id, route_date, customers_with_coords, needed_prospects, max_distance_km=5.0, exclude_custnos=None):
        ...
        try:
            ...
            # Calculate distance from center point to each prospect using ORS or Haversine
            if config.ORS_CONFIG['enabled'] and len(all_prospects_df) > 0:
                # Use ORS Matrix API for batch distance calculation (more efficient)
                self.logger.info("Using ORS Matrix API to calculate distances to prospects...")

                # Build locations list: [center_point, prospect1, prospect2, ...]
                locations = [[center_lon, center_lat]]  # Center point first (ORS uses lon,lat)

                for _, prospect in all_prospects_df.iterrows():
                    locations.append([prospect['longitude'], prospect['latitude']])

                # Get distance matrix from ORS
                distance_matrix = self.get_ors_distance_matrix(locations)

                if distance_matrix is not None:
                    # Extract distances from center point (row 0) to all prospects (columns 1 to N)
                    distances = distance_matrix[0][1:].tolist()
                    all_prospects_df['distance_km'] = distances
                else:
                    # ORS failed, fallback to Haversine
                    if config.ORS_CONFIG['fallback_to_haversine']:
                        self.logger.warning("ORS API failed for prospect search, falling back to Haversine")
                        distances = []
                        for _, prospect in all_prospects_df.iterrows():
                        ...
                        self.logger.error("ORS API failed and fallback to Haversine is disabled")
                        return pd.DataFrame()
            else:
                # ORS disabled or no prospects, use Haversine
                distances = []
                for _, prospect in all_prospects_df.iterrows():
                    dist = self.haversine_distance(
                        center_lat, center_lon,
    ...
    def solve_tsp_nearest_neighbor(self, locations_df, start_lat=None, start_lon=None):
        """Solve TSP using nearest neighbor heuristic with ORS API or Haversine distance
        ...
        try:
            ...
            # Build complete location list for distance matrix
            # Format: [[lon, lat], [lon, lat], ...]  (ORS uses lon,lat order)
            all_locations = []
            location_mapping = []  # Track which index corresponds to which customer

            # Add starting location if provided (index 0 in matrix)
            has_start_location = start_lat is not None and start_lon is not None
            if has_start_location:
                self.logger.info(f"Using starting location: ({start_lat}, {start_lon})")
                all_locations.append([start_lon, start_lat])
                location_mapping.append({'type': 'start', 'index': None})

            # Add all customer locations
            for idx, row in unvisited.iterrows():
                all_locations.append([row['longitude'], row['latitude']])
                location_mapping.append({'type': 'customer', 'index': idx})

            # Get distance matrix using ORS API (if enabled) or fallback to Haversine
            distance_matrix = None

            if config.ORS_CONFIG['enabled']:
                self.logger.info(f"Fetching ORS distance matrix for {len(all_locations)} locations (1 start + {len(unvisited)} customers)...")
                distance_matrix = self.get_ors_distance_matrix(all_locations)

                if distance_matrix is None and config.ORS_CONFIG['fallback_to_haversine']:
                    self.logger.warning("ORS API failed, falling back to Haversine distance matrix")

            # Build Haversine distance matrix if ORS is disabled or failed
            if distance_matrix is None:
                self.logger.info(f"Building Haversine distance matrix for {len(all_locations)} locations...")
                distance_matrix = np.zeros((len(all_locations), len(all_locations)))

                for i in range(len(all_locations)):
                    for j in range(len(all_locations)):
                        if i != j:
                            # Extract lat/lon from all_locations (remember: ORS format is [lon, lat])
                            lon1, lat1 = all_locations[i]
                            lon2, lat2 = all_locations[j]
                            distance_matrix[i][j] = self.haversine_distance(lat1, lon1, lat2, lon2)

            # Now use the distance matrix for nearest neighbor algorithm
            # Create index mapping: matrix index -> dataframe index
            if has_start_location:
                start_matrix_idx = 0
                customer_start_idx = 1
            else:
                start_matrix_idx = 0  # Start from first customer
                customer_start_idx = 0

            # Find first customer (nearest to start location)
            if has_start_location:
                # Get distances from start location (matrix row 0) to all customers
                distances_from_start = distance_matrix[0][customer_start_idx:]
                first_customer_offset = np.argmin(distances_from_start)
                current_matrix_idx = customer_start_idx + first_customer_offset
                self.logger.info(f"First customer is {distances_from_start[first_customer_offset]:.2f} km from starting location")
            else:
                # Start from first customer
                current_matrix_idx = 0

            # Track visited indices in the matrix
            visited_matrix_indices = set([current_matrix_idx])

            # Add first customer to route
            first_customer_df_idx = location_mapping[current_matrix_idx]['index']
            route.append(unvisited.loc[first_customer_df_idx])

            # Build route using nearest neighbor with distance matrix
            while len(visited_matrix_indices) < len(unvisited):
                # Get distances from current location to all other locations
                distances_from_current = distance_matrix[current_matrix_idx].copy()

                # Set visited locations to infinity
                for visited_idx in visited_matrix_indices:
                    distances_from_current[visited_idx] = np.inf

                # Set start location to infinity (we don't visit it as a customer)
                if has_start_location:
                    distances_from_current[0] = np.inf

                # Find nearest unvisited customer
                nearest_matrix_idx = np.argmin(distances_from_current)

                # Verify it's a valid customer (not the start location and not already visited)
                if location_mapping[nearest_matrix_idx]['type'] != 'customer':
                    break  # No more customers to visit

                # Add to route
                nearest_customer_df_idx = location_mapping[nearest_matrix_idx]['index']
                route.append(unvisited.loc[nearest_customer_df_idx])

                # Update current position and visited set
                visited_matrix_indices.add(nearest_matrix_idx)
                current_matrix_idx = nearest_matrix_idx
