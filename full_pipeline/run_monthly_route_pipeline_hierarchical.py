...
class HierarchicalMonthlyRoutePipelineProcessor:
    ...
    def get_distributors_hierarchy(self, db):
        ...
        try:
            ...
            # Step 1: Get all distributors (optionally filtered by specific distributor_id)
            distributor_filter = ""
            ...
            if distributors_df is None or distributors_df.empty:
                self.logger.error("No distributors found in MonthlyRoutePlan_temp")
                return {}

            hierarchy = {}

            for _, distributor_row in distributors_df.iterrows():
                distributor_id = distributor_row['DistributorID']
                self.logger.info(f"Processing DistributorID: {distributor_id}")

                # Step 2: Get all sales agents for this distributor
                agents_query = f"""
                ...
                if agents_df is None or agents_df.empty:
                    self.logger.warning(f"No agents found for DistributorID: {distributor_id}")
                    continue
                ...
                for _, agent_row in agents_df.iterrows():
                    ...
                    # Step 3: Get all dates for this agent (ordered chronologically)
                    dates_query = f"""
                    SELECT
                        RouteDate,
                    ...
                    GROUP BY RouteDate
                    ORDER BY RouteDate ASC  -- Chronological order
                    """
                ...
                total_agents = len(hierarchy[distributor_id])
                total_combinations = sum(len(dates) for dates in hierarchy[distributor_id].values())
        ...
        try:
            ...
            for date_info in sorted_dates:
                ...
                if not all_data_for_tsp.empty:
                    ...
                    optimized_data = self.solve_tsp_nearest_neighbor(all_data_for_tsp, self.start_lat, self.start_lon)
                        ...
                        if is_prospect:
                            ...
                            for optimized_data in all_optimized_data:
                                ...
                                if not matching_rows.empty:
                                    ...
                                    # Truncate Name to avoid SQL truncation error (max length varies by table schema)
                                    name_value = str(prospect_row.get('Name', ''))[:50]  # Truncate to 50 chars
                                    ...
                                    insert_params.append((
                                        ...
                                        name_value,
                                        wd_value,
                                        str(prospect_row.get('SalesManTerritory', ''))[:50],  # Truncate as well
                                        str(prospect_row.get('RouteName', ''))[:50],
                                        str(prospect_row.get('RouteCode', ''))[:50],
                                        str(prospect_row.get('SalesOfficeID', ''))[:50]
                                    ))
